...
// /api/items/:id, ING-031 added POST /api/items/photo, ING-032 added
// POST /api/items — still no delete).
...
// the approved write routes PUT /api/items/:id, POST /api/items/photo, and
// POST /api/items. No delete route and no unapproved method exists — any
...
// added POST /api/items.)
func TestING019_AC7_OnlyReadOnlyRoutes(t *testing.T) {
	engine, _, _ := newING019API(t)

	routes := engine.Routes()
	if len(routes) != 7 {
		t.Fatalf("registered routes = %d, want exactly 7: %v", len(routes), ing019RouteNames(routes))
	}
